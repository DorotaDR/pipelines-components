...
Number of time steps to forecast (horizon length). Positive integer (default: 1). top_n: Number of top models to select for the leaderboard and output (default: 3). backtest_num_windows: Number of rolling ``evaluate`` cutoffs on the hold-out test split during full refit (passed to
``autogluon_timeseries_models_full_refit``; default: 3).
